...
const {
  spendFreePoint: spendFreePointHelper,
  getHPLabel,
  getStaminaLabel,
  getReputationLabel,
  buildCharacterPanelData: buildCharacterPanelDataHelper
} = require('./panel-and-points');
...
function spendFreePoint(state, input) {
  return spendFreePointHelper(state, input, recalculateResources);
}
...
function buildCharacterPanelData(state) {
  return buildCharacterPanelDataHelper(state, {
    getPlayerLevel,
    getXPToNextLevel,
    BACKGROUNDS,
    REGIONS
